...
class MessageSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance):
        ...
        if instance.media:
            ...
            rep['media'] = request.build_absolute_uri(instance.media.url) if request \
...
class ChatSerializer(serializers.ModelSerializer):
    ...
    messages      = MessageSerializer(many=True, read_only=True,
                                      context={'request': None})  # обновим в view
    ...
    class Meta:
        ...
        read_only_fields = ['id', 'participants_usernames',
                            'current_user', 'created_at', 'messages']

    def get_current_user(self, obj):          #  <<<
        req = self.context.get('request')
        return req.user.id if req else None   #  <<<
